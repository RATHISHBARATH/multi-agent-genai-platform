...
from api.services.ppt_creator import create_pptx
from api.db import sync_session, init_db
from api.models.models import Task
import json, datetime, uuid, logging

logger = logging.getLogger('autoscillab.tasks.ppt')

@celery_app.task(name='task.generate_ppt', bind=True)
def generate_ppt(self, title, slides):
    try:
        init_db()
    except Exception:
        pass
    ...
    with sync_session() as session:
        t = Task(task_id=task_id, job_type='ppt', params=json.dumps({'slides': slides}), status='running', created_at=datetime.datetime.utcnow())
        session.add(t); session.commit()
    try:
        path = create_pptx(title, slides)
        with sync_session() as session:
        ...
        logger.exception('ppt task failed: %s', e)
        with sync_session() as session:
            t = session.query(Task).filter(Task.task_id==task_id).first()
